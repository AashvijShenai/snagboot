...
def run_firmware_child(dev, usb_vid, usb_pid):
...
def main():
	...
	devices = get_usb(usb_vid, usb_pid, recovery_config["multiflash"] == "true")
	
	with Pool() as pool:
		pool.starmap(run_firmware_child, [(x, usb_vid, usb_pid) for x in devices])
